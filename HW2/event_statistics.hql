...
-- ***** your code below *****
FROM events 
WHERE NOT EXISTS (SELECT * FROM mortality where events.patient_id = mortality.patient_id);
...
-- ***** your code below *****
FROM (SELECT count(event_id) AS event_count FROM alive_events GROUP BY patient_id) alive_count;
...
SELECT avg(event_count), min(event_count), max(event_count)
-- ***** your code below *****
FROM (SELECT count(event_id) AS event_count FROM dead_events GROUP BY patient_id) dead_count;
...
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT(time)) AS encounter_count FROM alive_events GROUP BY patient_id) alive_encounter;
...
SELECT avg(encounter_count), percentile(encounter_count, 0.5), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM(SELECT COUNT(DISTINCT(time)) AS encounter_count FROM dead_events GROUP BY patient_id) dead_encounter;
...
-- ***** your code below *****
FROM (SELECT DATEDIFF(MAX(time),MIN(time)) AS record_length FROM alive_events GROUP BY patient_id) alive_length;
...
SELECT avg(record_length), percentile(record_length, 0.5), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT DATEDIFF(MAX(time),MIN(time)) AS record_length FROM dead_events GROUP BY patient_id) dead_length;
